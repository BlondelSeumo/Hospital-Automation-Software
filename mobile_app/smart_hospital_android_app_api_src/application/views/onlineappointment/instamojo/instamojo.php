...
<title>Instamojo  Custom Integration</title>
...
<div class="container">
	...
	<div class="row">
		<div class="col-md-12">
			<div class="paybox">   
			    <div class="paybox_bg"> 
			        <h3 class="bt_title">Instamojo Payment Gateway</h3>  
			          <?php if($api_error){
                                ?>
                                <div class="alert alert-danger"><?php echo "<pre>"; print_r($api_error)?> </div>
                                <?php
                            }?>                       
                        <div class="paybody">
 
                                <form class="paddtlrb" action="<?php echo site_url('onlineappointment/instamojo/pay') ?>" method="POST" id="paypalForm">
									  ...
									  <div class="col-sm-8">
										...
										<div class="form-group row">
											<label class="col-sm-4">Amount(<?php echo $session_params['invoice']->symbol;?>)</label>
										  	<div class="col-sm-8"><input class="form-control" disabled="disabled" value="<?php echo set_value('amount', number_format((float)($this->session->userdata('payment_amount')), 2, '.', '')) ?>"/>
										  	</div>
										</div><!--./form-group-->
